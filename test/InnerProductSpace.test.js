...
describe('InnerProductSpace', () => {
  it('throws an error if no arguments are given', () => {
    expect(() => { new InnerProductSpace() }).toThrowError('wrong number of arguments (given 0, expected 1)');
  });

  it('throws an error if given a non-Number argument', () => {
    expect(() => { new InnerProductSpace('yes') }).toThrowError(/wrong argument type, expected Number/);
  });
